...
from finite_horizon_control.fhc_pipeline import (
    solve_ctrl_pb_incrementally,
    make_env,
)
...
def recorded_example(exp="pendulum"):
    if exp == "pendulum":
        env_cfg = dict(env="pendulum", horizon=40, stay_put_time=1.0)
...
def plain_example(exp="pendulum"):
    if exp == "pendulum":
        env = Pendulum(horizon=40, stay_put_time=1.0)
    elif exp == "cart_pendulum":
        env = CartPendulum(
            horizon=50, stay_put_time=0.6, x_limits=(-2.0, 2.0), dt=0.05
        )
    elif exp == "simple_car":
        env = Car(
            model="simple",
