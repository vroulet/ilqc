"""Backward passes for nonlinear control algorithms."""

import torch


def lin_quad_backward(
    ...
    """Linear quadratic backward pass.

    Compute optimal policies associated to the linear quadratic control problem approximating the environment at the
    current iteration using linear quadratic approximations.

    A detailed mathematical presentation is presented in
    [Iterative Linear Quadratic Optimization for Nonlinear Control: Differentiable Programming Algorithmic Templates,
      Roulet et al, 2022](https://arxiv.org/pdf/2207.06362)

    Args:
      traj: states computed along the input sequence of controls
      costs: costs computed along the input sequence of controls
      reg_ctrl: regularization to add on the control variables
      reg_state: regularization to add on the state variables (akin to a proximal point method)
      handle_bad_dir: how to handle the problem if the given costs are not convex such that the resulting
    ...
    """
    horizon = len(traj) - 1
    J, j, jcst = costs[-1].hess_state, costs[-1].grad_state, 0.0
    ...
    for t in range(horizon - 1, -1, -1):
        ...
        J, j, jcst, K, k, feasible = bell_step(
            A, B, J, j, jcst, P, p, Q, q, handle_bad_dir=handle_bad_dir
        )
...
def quad_backward(
    ...
    """Compute optimal policies associated to the linear quadratic approximation of the problem.
    ...
    Args:
      ...
      handle_bad_dir: how to handle the problem if the given costs are not convex such that the resulting
        subproblem cannot be computed simply by dynamic programming, see function bell_step
      mode: whether to instantiate the linear quadratic control problem as done by a Newton step or as done in a
    ...
    for t in range(horizon - 1, -1, -1):
        ...
        if t > 0:
            ...
            P = (
                P
                + traj[t + 1].hess_dyn_state(lam)
                + reg_state * torch.eye(P.shape[0])
            )
        R = traj[t + 1].hess_dyn_state_ctrl(lam)
        Q = (
            Q
            + traj[t + 1].hess_dyn_ctrl(lam)
            + reg_ctrl * torch.eye(Q.shape[0])
        )

        J, j, jcst, K, k, feasible = bell_step(
            A, B, J, j, jcst, P, p, Q, q, R, handle_bad_dir=handle_bad_dir
        )
        gain = dict(gain_ctrl=K, offset_ctrl=k)
        gains.insert(0, gain)
        ...
        if not feasible:
            break
    if handle_bad_dir == "check_total_value":
        feasible = jcst < 0
    return gains, jcst, feasible


def quad_backward_ddp(
    traj, costs, reg_ctrl=0.0, reg_state=0.0, handle_bad_dir="flag"
):
    """Instantiation of the backward pass for ddp oracle."""
    return quad_backward(
        traj, costs, reg_ctrl, reg_state, handle_bad_dir, mode="ddp"
    )


def quad_backward_newton(
    traj, costs, reg_ctrl=0.0, reg_state=0.0, handle_bad_dir="flag"
):
    """Instantiation of the backward pass for newton oracle."""
    return quad_backward(
        traj, costs, reg_ctrl, reg_state, handle_bad_dir, mode="newton"
    )
...
def bell_step(
    ...
    r"""Compute cost-to-go for linear quadratic approximation of the problem at a given time step.

    In other words, solve the Bellman equation associated to a linear quadratic control problem.
    Namely, given the cost-to-go at time t+1, the linear dynamic
    and the qudratic cost defined as, respectively,
    ...
    Args:
      ...
      J: :math: `J_{t+1}`
      j: :math: `j_{t+1}`
      jcst: :math: `j_{t+1, cst}`
      P: see above
      p: see above
      Q: see above
      q: see above
      R: see above
      handle_bad_dir: the subproblem defining :math: `c_t` may not be strongly convex in u 
        which make the solution potentially invalid. To check if the given solution is good,
        we simply check whether the rest of the minimization is negative. If it is not, we 
        either flag it or we modify the associated quadratic function or we just let it be.
    ...
    if rest > 0:
        if handle_bad_dir == "flag":
            ...
            feasible = True
            next_jcst = jcst + rest
        elif handle_bad_dir == "check_total_value":
            feasible = True
            next_jcst = jcst + rest
        else:
            raise NotImplementedError
