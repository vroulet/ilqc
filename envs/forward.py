"""Differentiable control environments and associated forward passes."""
...
class DiffEnv:
    """Differntiable control environment.
    ...
    def discrete_dyn(
        ...
        Args:
          state: current state
          ctrl: applied control variable
          time_iter: time index
    ...
    def costs(
        ...
        Args:
          next_state: next state computed by one step of the environment
          ctrl: applied control variable
          time_iter: time index
    ...
    def step(
        self, ctrl: torch.Tensor, approx: Optional[str] = None
    ) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
          ctrl: action taken at this step
          approx: flag to know if the intermediate first/second order information needs to be stored
    ...
    def forward(
        ...
        Args:
          cmd: sequence of controls to be applied on the system given as a tensor of size (nb_steps, dim_ctrl)
          approx: what approximations of the dynamics/costs need to be stored (see function step)
          reset: whether to reset the initial state or use the current state of the system
        ...
        state = self.state
        traj = [state]
        init_cost = torch.tensor(0.0)
        init_cost.hess_state, init_cost.grad_state = torch.zeros(
            dim_state, dim_state
        ), torch.zeros(dim_state)
        costs = [init_cost]
    ...
    def reset(self, requires_grad: bool = False) -> torch.Tensor:
        """Reset the environment to its initial state and its initial time index.

        Args:
          requires_grad: whether gradients associated to the state variables need to be computed
    ...
    def visualize(self, cmd: torch.Tensor, dt: Optional[float] = None) -> None:
        """Visualize the movement associated to a sequence of control variables.
        cmd: sequence of controls to be applied on the system given as a tensor of size (nb_steps, dim_ctrl)
        dt: time step to visualize the movement (default is to use the time step defined in the environment)
        """
        if dt is None:
            dt = self.dt
        self.reset()
...
def diff_discrete_dyn(
    discrete_dyn: Callable[[torch.Tensor, torch.Tensor, int], torch.Tensor],
    state: torch.Tensor,
    ctrl: torch.Tensor,
    time_iter: int,
    quad_approx: bool = False,
) -> torch.Tensor:
    """Make one step and record first/second order oracles.

    Args:
      discrete_dyn: discrete dynamics to compute derivatives from
      state: state of the environment
      ctrl: control variables of the environment
      time_iter: time index
      quad_approx: whether to record quadratic approximation

    Returns:
      next state filled with first/second order information of the dynamic w.r.t. the ctrl and the state
    """
    next_state = discrete_dyn(state, ctrl, time_iter)

    grad_dyn_state = auto_multi_grad(
        next_state, state, create_graph=quad_approx
    )
    grad_dyn_ctrl = auto_multi_grad(next_state, ctrl, create_graph=quad_approx)
    for attr, val in zip(
        ["grad_dyn_state", "grad_dyn_ctrl"], [grad_dyn_state, grad_dyn_ctrl]
    ):
...
def diff_cost(
    ...
    """Compute costs associated to new state and control used, record linear quadratic approximation of costs.

    Args:
      costs: pre-defined costs for the given environment
      next_state: state that has been computed by oen step of the dynamic
      ctrl: applied control variable
      time_iter: time index

    Returns:
      cost associated to the new state and the control applied to the system, gradients and hessians of the
    ...
    if cost_next_state.grad_fn is not None:
        cost_grad_state = auto_grad(
            cost_next_state, next_state, create_graph=True
        )[0]
        ...
        cost.hess_state, cost.grad_state = torch.zeros(
            dim_state, dim_state
        ), torch.zeros(dim_state)
    ...
    if cost_ctrl.grad_fn is not None:
        ...
        cost.hess_ctrl, cost.grad_ctrl = torch.zeros(
            dim_ctrl, dim_ctrl
        ), torch.zeros(dim_ctrl)
