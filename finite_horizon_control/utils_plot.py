...
def get_nice_writing(alternative=False):
    ...
    step_mode_names = dict(
        dir="dir", reg="reg", dirvar="dir var", regvar="reg var"
    )
    nice_writing.update(
        {
            algo + "_" + step_mode: algo_name + " " + step_mode_name
    ...
    if alternative:
        ...
        nice_writing.update(
            simple_car="Simple Model of Car\nwith Tracking Cost"
        )
        nice_writing.update(
            real_car="Bicycle Model of Car\nwith Tracking Cost"
        )
...
def get_palette_line_styles():
    algos_refs = [
        algo_type + "_" + approx + "_" + step_mode_name
    ...
    palette['classic_quad_dir'] = cmap[9]
    palette['ddp_quad_reg'] = cmap[8]
    palette['gd'] = cmap[7]
    # line_styles = {key: value for key, value in zip(algos_refs, line_styles_list)}
    # line_styles.update({'gd': (0, ())})
    marker_styles = {
        key: value for key, value in zip(algos_refs, marker_styles_list)
    }
...
def get_var_palette_line_styles():
    ...
    var_palette.update(
        {
            var_algos_refs[i]: cmap[(i + 2) % len(var_algos_refs)]
            for i in range(len(var_algos_refs))
        }
    )
    # var_line_styles.update({var_algos_refs[i]: line_styles_list[(i+2)%len(var_algos_refs)]
    #                         for i in range(len(var_algos_refs))})
    var_marker_styles.update(
        {
            var_algos_refs[i]: marker_styles_list[
                (i + 2) % len(var_algos_refs)
            ]
            for i in range(len(var_algos_refs))
        }
    )
