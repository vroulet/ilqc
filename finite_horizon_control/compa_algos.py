...
from finite_horizon_control.utils_plot import (
    nice_ax,
    get_nice_writing,
    set_plt_params,
    get_palette_line_styles,
)
...
def compa_algos_env(
    ...
    for optim_cfg in optim_cfgs:
        ...
        rates = [
            conv_algo["cost"][i + 1] / conv_algo["cost"][i]
            for i in range(len(conv_algo["cost"]) - 1)
        ] + [1e-12]
        conv_algo.update({"rate": rates})
        if revert_rates:
            conv_algo["rate"] = conv_algo["rate"][::-1]
        compa_optim = pd.concat(
            [compa_optim, DataFrame(conv_algo)], ignore_index=True
        )
    ...
    if y_axis == "cost":
        ...
        if normalize:
            aux = [
                (info - min_info + 1e-12)
                / (compa_optim[y_axis].to_list()[0] - min_info + 1e-12)
                for info in compa_optim[y_axis]
            ]
        else:
            aux = [
                info / compa_optim[y_axis].to_list()[0]
                for info in compa_optim[y_axis]
            ]
...
total_times = dict(
    pendulum=2.0, cart_pendulum=2.5, simple_car=2.0, real_car=1.0
)


def compa_algos_env_horizons(
    env_cfg, algos, horizons, max_iter, x_axis, y_axis, add_legend=True
):
    set_plt_params()
    ...
    for j, horizon in enumerate(horizons):
        ...
        optim_cfgs = [
            dict(max_iter=max_iter, algo=algo) for algo in algos_compared
        ]
    ...
    fig.suptitle(nice_writing[env_cfg["env"]], y=1.0)
    fig.tight_layout()
    if add_legend:
...
def compa_algos_env_stepsizes(
    ...
    for i, step_mode in enumerate(["dir", "reg"]):
        for j, horizon in enumerate(horizons):
            ...
            compa_optim = compa_optim[
                compa_optim["iteration"] >= start_iter_plot
            ]
            compa_optim = compa_optim[
                compa_optim["iteration"] <= max_iter_plot
            ]
            ...
            _, labels = axs[i][j].get_legend_handles_labels()
...
def plot_conv_all_algos():
    env_cfgs = [
        dict(env="pendulum"),
        dict(env="cart_pendulum", x_limits=(-2.0, 2.0), stay_put_time=0.6),
        dict(
            env="simple_car",
            track="simple",
            cost="exact",
    ...
    for approx in ["linquad", "quad"]:
        for x_axis in ["iteration", "time"]:
            for env_cfg in env_cfgs:
                ...
                algos = algos + [
                    algo_type + "_" + approx + "_" + step_mode
                    for algo_type in ["classic", "ddp"]
                    for step_mode in ["reg", "dir"]
                ]

                fig = compa_algos_env_horizons(
                    env_cfg,
                    algos,
                    horizons,
                    max_iter,
                ...
                fig.tight_layout()
                name_fig = "_".join([env_cfg["env"], approx, x_axis, y_axis])
                file_name = f"plots/{name_fig}.pdf"
    ...
    for approx in ["linquad", "quad"]:
        for env_cfg in env_cfgs:
            algos = [
                algo_type + "_" + approx for algo_type in ["classic", "ddp"]
            ]
            fig = compa_algos_env_stepsizes(
                env_cfg,
                algos,
                horizons,
                max_iter,
...
def plot_conv_gd_ilqr_iddp():
    ...
    env_cfgs = [
        dict(env="pendulum", reg_ctrl=0.0, horizon=100, dt=2.0 / 100),
        dict(
            env="simple_car",
            track="simple",
            cost="exact",
    ...
    fig.tight_layout()
    fig.savefig(
        "plots/plot_conv_gd_ilqr_iddp.pdf", format="pdf", bbox_inches="tight"
    )
...
def plot_conv_gd_ilqr_ddp_real_car():
    ...
    fig.tight_layout()
    fig.savefig("plots/hard_conv.pdf", format="pdf", bbox_inches="tight")
...
def plot_rates_and_min_sev():
    ...
    ax = sns.lineplot(
        x=x_axis, y=y_axis, data=metrics, color=palette[algo], ax=axs[0]
    )
    ax.set_xlabel(nice_writing[x_axis])
    ax.set_ylabel(nice_writing[y_axis])
    ax.set_yscale("log")
    ...
    ax.set_xticks([3.0, 10.0, 30.0, 80.0])
    ...
    fig.tight_layout()
    fig.savefig("plots/min_sev_rates.pdf", format="pdf", bbox_inches="tight")
